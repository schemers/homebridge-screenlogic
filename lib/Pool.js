    ...
    return new Promise(function(resolve, reject) {
      connection
        ...
        .once('badParameter', function() {
          reject(Controller.badParameterError)
        })
    ...
    return new Promise(function(resolve, reject) {
      ...
      remote.on('gatewayFound', function(unit) {
        ...
        if (unit && unit.gatewayFound) {
          ...
          reject(Controller.noRemoteUnitFoundError)
        }
      })
      remote.connect()
    })
...
Controller.loginError = Error('unable to login')
Controller.noRemoteUnitFoundError = Error('no remote unit found')
Controller.noBroadcastUnitFoundError = Error('no unit found via broadcast')
Controller.badParameterError = Error('bad parameter passed to set command')
...
class Status {
  constructor(status) {
    ...
    for (const circuit of status.circuitArray) {
      this.circuitState[circuit.id] = circuit.state
    }
  }
}
